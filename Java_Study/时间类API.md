...
- `public String format(Date date)`：将Date对象格式化为字符串。//用SDF对象去格式化Date对象d1转为指定格式字符串(日期对象 ->字符串)

- `public Date parse(String source)`：将字符串解析为Date对象。//用SDF对象去解析字符串转为Date对象(日期对象 ->字符串)
  ...
  public class SDF {
      public static void main(String[] args) throws ParseException {
          ...
          */
          ...
          Date date = sdf.parse(str);//用SDF对象去解析字符串转为Date对象(日期对象 ->字符串)
          //3.打印结果
          System.out.println(date.getTime());//1731294671000
  
          //用SDF对象去格式化Date对象d1转为指定格式字符串(日期对象 ->字符串)
          Formatmethod();
      ...
      public static void Formatmethod() {
          ...
          Date d1 = new Date(0L);
          String str1 = sdf1.format(d1);//用SDF对象去格式化Date对象d1转为指定格式字符串(日期对象 ->字符串)
          System.out.println(str1);
  
          //带参构造, 指定格式(不会的格式就去查API文档
          SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy年MM月dd日 HH:mm:ss EEEE");
          String str2 = sdf2.format(d1);//用SDF对象去格式化Date对象d1转为指定格式字符串(日期对象 ->字符串)
...
public class SDFTest2 {
    public static void main(String[] args) throws ParseException {
        ...
        //定义字符串表示三个时间，解析三个时间，得到Date对象，get得到三个时间的毫秒值
...
- 有两种方式可以获取GregorianCalendar对象：
  ...
  - **通过**Calendar的静态方法**getInstance()方法获取**GregorianCalendar**对象**【本次课使用】
...
public class Demo {
    public static void main(String[] args) {
        ...
        //计算200天以前是哪年哪月哪日，星期几？
        //c2.add(Calendar.DAY_OF_MONTH, -200);//日期减200
...
![JDK8新增时间相关类的优点](./images/TimeClass/JDK8%E6%96%B0%E5%A2%9E%E6%97%B6%E9%97%B4%E7%9B%B8%E5%85%B3%E7%B1%BB%E7%9A%84%E4%BC%98%E7%82%B9.png)
...
Instant instant3 = Instant.ofEpochSecond(1L, 1000000000L);// 1秒 加 1000000000纳秒 等于 2秒
...
```java
//public Integer(int value)：根据 int 值创建 Integer 对象(JDK5过时)
...
在 JDK5 以后, int 和 Integer 可以看作同一个东西, 因为两者在内部可以自动转化
...
除了Character类之外，其他所有包装类都具有parseXxx静态方法可以将字符串参数转换(**解析**)为对应的基本类型：
...
public class Demo3 {
    public static void main(String[] args) {
        int decimalNum = 11;
        String binaryNum = toBinaryString(decimalNum);
        System.out.println(binaryNum);
    }
    //核心逻辑:
    //不断的去 %2，得到余数，再 /2, 一直到商为0就结束。
    //还需要把余数倒着拼接起来
