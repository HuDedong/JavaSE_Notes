...
7. [a-dm-p]：a 到 d 或 m 到 p之间的任意一个字符。 和↑相同
...
- 语法示例：
  ...
  4. "\s"： 空白字符：[ \t\n\x0B\f\r] 的简写
  5. "\S"： 非空白字符：\[^\s\] 的简写
  6. "\w"：英文 _ 0到9的字符：[a-zA-Z_0-9]的简写
  7. "\W"：非英文 _ 0到9的字符：\[^\w\]
- 代码示例：
...
- 语法示例：下面的 X 表示任意内容
  ...
  7. 注意:       "something(FLAG){n,m}"  用小括号分组, 表示FLAG能出现n到m次(n和m都是包含的)FLAG可以是任何东西
  
  + 注意:       `\\w&&[^_]{}是错的, 只有再加一层中括号: [\\w&&[^_]]才是对的`
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        /*定义正则表达式
          ...
          但是在获取的时候，只获取前半部分
        */
        //忽略大小写一并读取用((?i)something)

        String regex1 = "((?i)Java)(?=8|11|17)";//小括号不是中括号
        String regex2 = "((?i)Java)(?:8|11|17)";//也可以: "((?i)Java)(8|11|17)"
        String regex3 = "((?i)Java)(?!8|11|17)";

        printRegex(regex1, str);
        printRegex(regex2, str);
        printRegex(regex3, str);
    }
    public static void printRegex(String regex, String str) {
        Pattern p = Pattern.compile(regex);
        Matcher m = p.matcher(str);
        while (m.find()) {
...
## 1.13 String类的replaceAll方法中使用正则表达式
...
split方法是把原字符串中的 regex 删掉后剩下的几段存入 String[] 中, 字符串本身没有变(只是一种操作, 引用类型自己不变)
...
**只看左括号**，不看右括号，**按照左括号的顺序，从左往右，依次为**第一组，第二组，第三组等等

```java
public class CatchRegex {
    public static void main(String[] args) {
        //需求1:判断一个字符串的开始字符和结束字符是否一致?只考虑一个字符
        //举例: a123a b456b 17891 &abc& a123b(false)
        //末尾写 "\\组号" 表示把第X组的内容再出来用一次
        String regex1 = "(.).+\\1";
        ...
        //需求3:判断一个字符串的开始部分和结束部分是否一致?开始部分内部每个字符也需要一致
            //举例: aaa123aaa bbb456bbb 111789111 &&abc&& aaa123aab(false)
        //(.):把首字母看做一组
        // "(.+)\\1*" 表示取第一个字符，*为0到多次
        // 因为看左括号顺序,所以 \\1 变 \\2
        // \\2:把首字母拿出来再次使用
